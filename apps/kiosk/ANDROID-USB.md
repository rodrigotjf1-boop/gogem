# Transporte USB Android (TM-T88VII) — plugin de referência

Após `flutter create`, cole a classe abaixo em
`android/app/src/main/kotlin/br/com/dms/gogem_kiosk/UsbPrinterPlugin.kt`
e registre no `MainActivity.kt` (configureFlutterEngine → adicionar o handler).
Adicione ao `AndroidManifest.xml` (dentro de <activity>):
```xml
<intent-filter><action android:name="android.hardware.usb.action.USB_DEVICE_ATTACHED"/></intent-filter>
<meta-data android:name="android.hardware.usb.action.USB_DEVICE_ATTACHED" android:resource="@xml/device_filter"/>
```
e crie `android/app/src/main/res/xml/device_filter.xml`:
```xml
<resources><usb-device vendor-id="1208"/></resources>  <!-- 0x04B8 = Epson -->
```
...
/** Canal `gogem/usb_printer`: open/close/write/read + EventChannel ASB. */
class UsbPrinterPlugin(private val ctx: Context) {
  ...
  fun registrar(engine: FlutterEngine) {
    MethodChannel(engine.dartExecutor.binaryMessenger, "gogem/usb_printer")
      .setMethodCallHandler { call, result ->
        ...
        } catch (e: Exception) { result.error("USB", e.message, null) }
      }
    EventChannel(engine.dartExecutor.binaryMessenger, "gogem/usb_printer/asb")
      .setStreamHandler(object : EventChannel.StreamHandler {
        ...
        override fun onCancel(a: Any?) { asbSink = null }
      })
  }
  ...
  private fun abrir(): Boolean {
    ...
    if (!mgr.hasPermission(dev)) return false // pedir permissão via intent no provisionamento
  ...
  private fun fechar() { lendo = false; iface?.let { conn?.releaseInterface(it) }; conn?.close(); conn = null }
  ...
  private fun ler(max: Int, timeoutMs: Int): ByteArray {
    ...
    val n = conn?.bulkTransfer(epIn, buf, max, timeoutMs) ?: -1
  ...
  private fun iniciarLeituraAsb() {
    ...
    Thread {
      ...
      while (lendo && conn != null) {
        val n = conn?.bulkTransfer(epIn, buf, 4, 500) ?: -1
        if (n == 4) {
...
⚠️ NOTA DE BANCADA: com a thread de ASB ativa, as leituras síncronas de
DLE EOT competem pelo endpoint IN. No bench da F4-hardware, validar e — se
necessário — pausar a thread ASB durante `read()` (mutex simples). As máscaras
de bits de status (ver `packages/escpos/lib/src/status.dart`) também devem ser
confirmadas com a TM-T88VII real no totem nº 1.
